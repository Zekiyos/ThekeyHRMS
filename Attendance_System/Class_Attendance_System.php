...
// return the difference
    ...
    public function Working_Hour($Scan_ID, $Scan_Department, $Scan_Date, $Scan_Start, $Scan_Start_Break, $Scan_End_Break, $Scan_End, $late_tolerance = NULL) {
        ...
        else {  // who has not break time    //Spray case for Sher which only Start is avail
            if (($Start_Break == "00:00:00") and ($End_Break == "00:00:00") and ($End == "00:00:00")) {
                if ($Scan_Start > "00:00:00") {
                    if ($Scan_Start <= $Start) {
                        $First_Session_Start = $Start;

                        $First_Session_Start_AB = "00:00:00";

                        $First_Session_HR = "08:00:00";
                    } else
                    if ($Scan_Start > $Start) {
                        $First_Session_Start = $Scan_Start;
                ...
                $Second_Session_Start = "NAx";
                $Second_Session_End = "NA";
                $Second_Session_HR = "NA";

                $WorkingHR = $First_Session_HR;
            }//if only breaks are not avaial
        ...
        if (strtoupper(substr($Scan_Department, 9, 9)) == strtoupper('Cold Room')) {
            if ($Scan_Start <= $Start) {
                $First_Session_Start = $Start;
            } else
            if ($Scan_Start > $Start) {
            ...
            if ($Scan_End_Break < $End) {
                $First_Session_End = $Scan_End_Break;
            }
            ...
            if (($Scan_Start_Break != "00:00:00") and ($Scan_End_Break != "00:00:00")) {
                ...
                $Second_Session_Start = "NAZ";
                $Second_Session_End = "NA";
                $Second_Session_HR = "NA";
            }

            $WorkingHR = $First_Session_HR;
        } // end of coldroom('GH 11-12 Cold Room') case //if only breaks are not avaial
...
Sum(Time_To_Sec( attendance_allocation.HolydayOT_HR)) /3600 As HolyDayOT_Hour 
 ...
 Order By attendance_allocation.Department,attendance_allocation.ID";
        ...
        $DB_PASSWORD = 'EWSadmin'; //EWSadmin';//$password_HRMS;//'';//'EWSadmin';
