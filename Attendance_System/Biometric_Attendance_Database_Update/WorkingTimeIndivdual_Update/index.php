        ...
        <div id="thekey_page">
            ...
            <div id="mainContent"><!-- InstanceBeginEditable name="MainContent" -->
                ...
                if (isset($_GET['Auto_ID']) && (count($_POST) == 0)) {
                    ...
                    $result = $mydb->delete('working_time_setting_individual');
                    if ($result) {
                ...
                $mydb = new DataBase();
                $header_lable = 'Individual Working Time Setting';
                $mydb = filter($mydb);
                $result = $mydb->get('working_time_setting_individual');
                $total_num_row = $result['total_row'];                 $result = $result['result'];

                $header_info = $mydb->show_column('working_time_setting_individual');
                ksort($header_info);

                $update_page = 'WorkingTimeIndivdualUpdate.php';
                $key_filed = 'Auto_ID';

                $hide = array('Auto_ID' => 'Auto_ID');
                $table = "working_time_setting_individual";
