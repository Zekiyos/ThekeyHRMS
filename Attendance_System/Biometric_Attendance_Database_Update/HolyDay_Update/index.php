        ...
        <div id="thekey_page">
            ...
            <div id="mainContent"><!-- InstanceBeginEditable name="MainContent" -->
                ...
                if (isset($_GET['Auto_ID']) && (count($_POST) == 0)) {
                    ...
                    $result = $mydb->delete('holyday_definition');
                    if ($result) {
                ...
                $mydb = new DataBase();
                $header_lable = 'Holyday Definition';
                $mydb = filter($mydb,true);
                $result = $mydb->get('holyday_definition');
                $total_num_row = $result['total_row'];                 $result = $result['result'];
                
                $header_info = $mydb->show_column('holyday_definition');
                ksort($header_info);

                $update_page = 'HolydayUpdate.php';
                $key_filed = 'Auto_ID';

                $hide = array('Auto_ID' => 'Auto_ID');
                $table = "holyday_definition";
