        ...
        <div id="thekey_page">
            ...
            <div id="mainContent"><!-- InstanceBeginEditable name="MainContent" -->
                ...
                if (isset($_GET['Auto_ID']) && (count($_POST) == 0)) {
                    ...
                    $result = $mydb->delete('ot_definition_department');
                    if ($result) {
                ...
                $mydb = new DataBase();
                $header_lable = 'Departmental Over Time Work Definition';
                $mydb = filter($mydb);
                $mydb = filter($mydb);
                $result = $mydb->get('ot_definition_department');
                $total_num_row = $result['total_row'];                 $result = $result['result'];
                
                $header_info = $mydb->show_column('ot_definition_department');
                ksort($header_info);

                $update_page = 'OTDepartmentUpdate.php';
                $key_filed = 'Auto_ID';

                $hide = array('Auto_ID' => 'Auto_ID');
                $table = "ot_definition_department";
