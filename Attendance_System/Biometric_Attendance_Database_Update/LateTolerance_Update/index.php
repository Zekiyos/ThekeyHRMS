        ...
        <div id="thekey_page">
            ...
            <div id="mainContent"><!-- InstanceBeginEditable name="MainContent" -->
                ...
                if (isset($_GET['Auto_ID']) && (count($_POST) == 0)) {
                    ...
                    $result = $mydb->delete('late_tolerance_setting');
                    if ($result) {
                ...
                $mydb = new DataBase();
                $header_lable = 'Late Tolerance Setting';
                $mydb = filter($mydb,true);
                $result = $mydb->get('late_tolerance_setting');
                $total_num_row = $result['total_row'];                 $result = $result['result'];
                
                $header_info = $mydb->show_column('late_tolerance_setting');
                ksort($header_info);

                $update_page = 'LateToleranceUpdate.php';
                $key_filed = 'Auto_ID';

                $hide = array('Auto_ID' => 'Auto_ID');
                $table = "late_tolerance_setting";
