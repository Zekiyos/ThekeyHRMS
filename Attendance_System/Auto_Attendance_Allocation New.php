...
//        $totalsec = $to_time + $from_time;
    ...
    public function Attendance_Allocation($From_Date, $To_Date, $Department = NULL, $ID = NULL, $operation = NULL) {
...
//		echo "<td>" . $rowAS['From_Date'] . "</td>";
 ...
 Order By attendance_allocation.Department,IDNO";
    ...
    public function run_slq_file() {
        ...
        $hostname_HRMS = "192.168.1.201";
        $database_HRMS = "ThekeyHRMSDB";
        $username_HRMS = "root";
        $password_HRMS = "EWSadmin"; //EWSadmin";//
...
//            echo "Attendance Scan Sheet Imported for $Department from $From_Date to $To_Date Successfully.\n";
//        else
//        if (($resultAS) && ($Department == NULL))
//            echo "Attendance Scan Sheet Imported from $From_Date to $To_Date Successfully.\n";
//        else
//            echo "Attendance Scan Sheet Not Imported for $Department from $From_Date to $To_Date Successfully.\n";
...
function connect() {
    ...
    if (!$con) {
        $myconn = @mysql_connect($hostName, $userName, $password);
        if ($myconn) {
...
$objAS = new Auto_Attendance_System();
connect();
...
//$fromDate = date('Y-m-d',$beforeYesterday);    
//$toDate = date('Y-m-d',$beforeYesterday); 

$filename = "Auto_Attendance_Allocation_log.txt";
$operation = "INSERT";
$departmentList = $objAS->getDepartment();

echo "******************************************************\n";
echo "***             Thekey HRMS                        ***\n";
echo "***            (Schedule Task)                     ***\n";
echo "***   Thekey HRMS Automatic Attendance Allocation  ***\n";
echo "***      From Date: $fromDate    To Date:  $toDate ***\n";
echo "******************************************************\n";

echo "*** S.N * Department    ** Total Running Time (Second)    **(Minute) ** (Hour) \n";

$count = 0;
$timeTotal = microtime();
$timeTotal = explode(' ', $timeTotal);
$timeTotal = $timeTotal[1] + $timeTotal[0];
$startTotal = $timeTotal;

foreach ($departmentList as $key => $Department) {
    $sql = "SELECT * FROM attendance_allocation WHERE Department='" . $Department . "' AND Date >='" . $fromDate . "' AND  Date<='" . $toDate . "'";
    // echo $sql;
    $result = mysql_query($sql);
    if (!(mysql_num_rows($result))) {
        ...
        $count++;
        $objAS->Attendance_Cleaner("attendance_sheet", $fromDate, $toDate, $Department);
        $objAS->Attendance_Scan_Sheet_Importer($fromDate, $toDate, $Department);

        $objAS->Attendance_Cleaner("attendance_allocation", $fromDate, $toDate, $Department);

        $objAS->Attendance_Allocation($fromDate, $toDate, $Department, NULL, $operation);
        ...
        echo "*** $count * $Department  ** $total_time   ** " . ($total_time / 60) . "  ** " . ($total_time / 3600) . "\n";

        if (file_exists($filename)) {
            $fd = fopen($filename, "r+") or die("Can't open file $filename");
            $fstring = fread($fd, filesize($filename));

            $fstring2 = $fstring . "*** $count * $Department  *  " . $fromDate . " * $total_time   ** " . ($total_time / 60) . "  ** " . ($total_time / 3600) . "\n";
            fclose($fd);
    ...
    $timeTotal = explode(' ', $timeTotal);
    $timeTotal = $timeTotal[1] + $timeTotal[0];
    $finishTotal = $timeTotal;
    $total_timeTotal = round(($finishTotal - $startTotal), 4);

    echo "***   * Total Excution Time: ** $total_timeTotal ** " . ($total_timeTotal / 60) . " ** " . ($total_timeTotal / 3600) . "\n";
}
    //   exit;
