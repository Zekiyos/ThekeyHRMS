...
attendance_allocation.MiddelName,attendance_allocation.LastName,
...
@Working_Day:=11-(IF((Sum(Time_To_Sec(if(isnull(attendance_allocation.AbsentHR),'00:00:00',attendance_allocation.AbsentHR))) /28800)
-
floor((Sum(Time_To_Sec(if(isnull(attendance_allocation.AbsentHR),'00:00:00',attendance_allocation.AbsentHR))) /28800) ) < 0.25,
...
Prepared,Department_Manager,Checked,Aproved

  From attendance_allocation 
  JOIN Department ON attendance_allocation.Department=Department.Department
  INNER JOIN User_Attendance ON Department.Section = User_Attendance.`Green House`
