...
-(IF(isnull(TotalOffDays),0,if(working_day_val>=23,0, TotalOffDays))))<0,0,(working_day_val -
(IF((Sum(Time_To_Sec(if(isnull(attendance_allocation.AbsentHR),'00:00:00',attendance_allocation.AbsentHR))) /28800)
-
floor((Sum(Time_To_Sec(if(isnull(attendance_allocation.AbsentHR),'00:00:00',attendance_allocation.AbsentHR))) /28800) ) < 0.25,
...
CEILING((Sum(Time_To_Sec(if(isnull(attendance_allocation.AbsentHR),'00:00:00',attendance_allocation.AbsentHR))) /28800)))))
-(IF(isnull(TotalLeaveDays),0, if(TotalLeaveDays>working_day_val,working_day_val,TotalLeaveDays) ))
-(IF(isnull(TotalOffDays),0,if(working_day_val>=23,0, TotalOffDays)))))

 AS Working_Day,

@TotalLeaveDays:=(IF(isnull(TotalLeaveDays),0,if(TotalLeaveDays>working_day_val,working_day_val-(IF(isnull(TotalOffDays),0,if(working_day_val>=23,0, TotalOffDays))),TotalLeaveDays))) AS Total_Leave_Days,

@TotalOffDays:=(IF(isnull(TotalOffDays),0,TotalOffDays)) AS Total_Off_Days,
