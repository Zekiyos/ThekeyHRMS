...
$query_RSALNotification = "SELECT * FROM annual_leave";
$RSALNotification = mysql_query($query_RSALNotification, $HRMS) or die(mysql_error());
$row_RSALNotification = mysql_fetch_assoc($RSALNotification);
$totalRows_RSALNotification = mysql_num_rows($RSALNotification);
?>
...
//' Use any basic <em>HTML.</em>';
				...
				slmg[0]='<font color="#03F"> <h3>Today\'s Notification </h3></font>' 
				 <?php 
				// DATEDIFF('ReportOn','$d')
                         $d=date("Y-m-d");
       			 	//$query  = "SELECT * FROM annual_Leave where ReportOn >= ".$d."";
					//$query  = "SELECT * FROM annual_Leave where  ReportOn=CURDATE()";
					//$query="SELECT ID, FirstName, MiddelName, LastName, ReportOn FROM annual_Leave WHERE ReportOn = CURDATE( )	UNION ALL SELECT ID, FirstName, MiddelName, LastName, ReportOn FROM sick_leave WHERE ReportOn = CURDATE( )";
					$query="SELECT ID, FirstName, MiddelName, LastName, ReportOn,LeaveType
FROM annual_Leave
WHERE ReportOn <= CURDATE( ) and Reported='NO'
UNION ALL
SELECT ID, FirstName, MiddelName, LastName, ReportOn,LeaveType
FROM sick_leave
WHERE ReportOn <= CURDATE( ) and Reported='NO'
UNION ALL
SELECT ID, FirstName, MiddelName, LastName, ReportOn,LeaveType
...
WHERE ReportOn <= CURDATE( ) and Reported='NO'";
				...
				while($row = mysql_fetch_array($result, MYSQL_ASSOC))
				{
					...
					//if ($row['ReportOn'] == date("Y-m-d"))
					//{
						// $content=$row['FirstName']." ".$row['MiddelName']." ".$row['LastName'];
						$i=$i+1;
			...
			slmg[<?php echo $i;?>]='<?php echo "<font color=\"#FF9900\" size=\"+1\" face=\"Times New Roman, Times, serif\">"."{$row['FirstName']}"." {$row['MiddelName']}"." {$row['LastName']}"."<br>";
				...
				{
					$LeaveType="Jaarlijks verlof";
				}
				if($row['LeaveType']=="Sick Leave")
				{
					$LeaveType="Ziekteverlof";
				}
				if($row['LeaveType']=="Maternity")
				{
					$LeaveType="Zwangerschapsverlof";
				}
				if($row['LeaveType']=="Funeral Leave")
				{
					$LeaveType="Laat begrafenis";
				}
			if($row['LeaveType']=="Wedding Leave")
				{
					$LeaveType="Laat bruiloft";
				}	
					echo "<font color=\"#FF3300\" size=\"+0\" >"."Klaar bent met zijn / haar ".$LeaveType." dagen op datum "."{$row['ReportOn']}".".He / zij moeten  <a href=\"Leaves/Back_From_Leave_Report.php?ID=".$row['ID']."\" >rapporteren</a> over vandaag" . "<p> </p>";?>';
					<?php
...
<?php
mysql_free_result($RSALNotification);
?>
