...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO Equipment_HandOver (ID, FirstName, MiddelName, LastName,EquipmentName, Taken_Date, Replacement_Date) VALUES ( %s,%s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['ID'], "text"),
                       GetSQLValueString($_POST['FirstName'], "text"),
                       GetSQLValueString($_POST['MiddelName'], "text"),
                       GetSQLValueString($_POST['LastName'], "text"),
					   GetSQLValueString($_POST['EquipmentName'], "text"),
                       GetSQLValueString($_POST['Day_Taken'], "date"),
                       GetSQLValueString($_POST['Replacement_Date'], "date")               );

  mysql_select_db($database_HRMS, $HRMS);
  $Result1 = mysql_query($insertSQL, $HRMS) or die(mysql_error());

echo "<script type=\"text/javascript\">alert('You have registed the Data Seccussfully.')</script>";
  //$insertGoTo = "../test.htm";
//  if (isset($_SERVER['QUERY_STRING'])) {
...
<li><a href="#" class="MenuBarItemSubmenu">Equipment Handover</a>
              <ul>
                <li><a href="Equipment_HandOver.php">Equipment Handover</a></li>
                <li><a href="Equipment_ReturnBack.php">Equipment Return Back</a></li>
              </ul>
            </li>
          </ul>
        </li>
...
<?php echo "<font face=\"Times New Roman, Times, serif\" size=\"+1\"><b>loged in as ".$_SESSION['MM_Username']."</b></font>";  ?>
  ...
  <div id="mainContent"><!-- InstanceBeginEditable name="MainContent" -->
    ...
    <blockquote>
     <blockquote>
      <blockquote>
       <font color="#FF6600" size="+1"> Equipment Handover Form </font>
    <?php include("Select_ID4EquipmentHandOver.php"); ?>
<?php
...
?>
    ...
    <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
      ...
      <table width="400" height="362" align="center" background="" bgcolor="#EBEBEB">
        <tr valign="baseline">
          <td width="128" height="39" align="right" nowrap="nowrap"> Selected ID:</td>
          <td width="385">
     <input name="ID" value="<?php if(isset($_GET['ID'] )){	echo $_GET['ID'];} ?>" size="10" readonly="readonly"  />     
        ...
        <tr valign="baseline">
          <td height="36"  align="right" nowrap="nowrap">Equipements:</td>
          <td>
            <select name="EquipmentName">
        <option>Please choose the Equipment</option>
        <!--option>Apron</option>
        <option>Bending Glove</option>
        <option>Cutter</option>
        <option>Dust Coat</option>
        <option>Face Mask</option>
        <option>Google</option>
        <option>Gum Boots</option>
        <option>Overall</option>
        <option>Lather Apron</option>
        <option>Rubber Glove</option>
        <option>Respiratory</option>
        <option>Small Glove</option>
        <option>Skimo Overall</option>
        <option>Welding Boots</option>
        <option>Welding Google</option-->
  <option style="background:#00F; color:#FFF" >Dust Coats(Blue)</option>
  <option >Dust Coats(White)</option>
  <option  style="background:#00F; color:#FFF">Eskimo Suits(Blue)</option>
  <option style="background: #FF0" >Overalls(Yellow)</option>
  <option style="background: #F90; color:#FFF">Overalls(Orange)</option>
  <option style="background: #F90; color:#FFF" >Overalls(Orange)</option>
  <option style="background: #F00; color:#FFF" >Special Overall(Red)</option>
  <option style="background: #FF0">Impermeable Suits(Yellow)</option>
  <option style="background: #000; color:#FFF">Gumboots(Black)</option>
  <option style="background: #000; color:#FFF"> PVC Hand Gloves(Black)</option>
  <option style="background: #630; color:#FFF">3m Masks( complete with filters) (Brown)</option>
  <option style="background:#00F; color:#FFF">Spray Mask (RC 203)(Yellow)</option>
<option >Filters (RC 209)(Colorless)</option>
        <option >Safety spray goggles(White)</option>
      <option style="background: #69F; color:#FFF">Short Leather Hand Gloves(Blue)</option>
      <option style="background:#00F; color:#FFF">Security uniform (Cap, Trousers, Coats)(Blue)</option>
     <option>Security Shoes(Black)</option>
     <option style="background:#00F; color:#FFF">Overall(Blue) </option>
    <option style="background:#00F; color:#FFF">Apron (Blue)</option>
  </select>
          </td>
        </tr>
        <tr valign="baseline">
          <td height="43"  align="right" nowrap="nowrap">Day Taken On:</td>
          <td><script type='text/JavaScript' src="../Calender/scw.js" ></script>
		
            <input name="Day_Taken" type="text" onclick='scwShow(this,event);' value="<?php echo date("Y-m-d"); ?>" size="20" maxlength="20" />
          </td>
        </tr>
        <tr valign="baseline">
          <td height="62" align="right" nowrap="nowrap">Replacement Date:</td>
          <td><script type='text/JavaScript' src="../Calender/scw.js" ></script>
		
		<input type="text" name="Replacement_Date" onclick='scwShow(this,event);' value='<?php echo date("Y-m-d");?>' />
</td>
        </tr>
 ...
 //$date = date("Y-m-d",strtotime($date)) 
        ...
        <tr valign="baseline">
          <td nowrap="nowrap" align="right">&nbsp;</td>
            <td><input type="submit" value="Register" onclick="return confirm('Are you sure you want to registered for this Employee?')"   /></td>
      </tr>
      </table>
      </font>
      <p>
