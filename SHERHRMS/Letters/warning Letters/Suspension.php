...
if ((isset($_POST["MM_insert"])) && isset($_GET['ID']) && ($_POST["MM_insert"] == "form1")) {
	
	//DELETE FROM table_name WHERE some_column=some_value
	
	
  $insertSQL = sprintf("INSERT INTO suspended_employee (ID,FirstName, MiddelName, LastName, Department, Suspension_Date,Suspension_Reason) VALUES (%s,%s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_GET['ID'], "text"),
                       GetSQLValueString($_POST['FirstName'], "text"),
                       GetSQLValueString($_POST['MiddelName'], "text"),
                       GetSQLValueString($_POST['LastName'], "text"),
                       GetSQLValueString($_POST['Department'], "text"),
                      GetSQLValueString($_POST['Suspension_Date'], "date"),
					  GetSQLValueString($_POST['Suspension_Reason'], "text") 
					  );
		

  mysql_select_db($database_HRMS, $HRMS);
  $Result1 = mysql_query($insertSQL, $HRMS) or die(mysql_error());
echo "<script type=\"text/javascript\">alert('{$_POST['FirstName'] } {$_POST['MiddelName']} is suspeneded sucessfully')</script>";

}

mysql_select_db($database_HRMS, $HRMS);
$query_RSSuspension = "SELECT * FROM suspended_employee";
$RSSuspension = mysql_query($query_RSSuspension, $HRMS) or die(mysql_error());
$row_RSSuspension = mysql_fetch_assoc($RSSuspension);
$totalRows_RSSuspension = mysql_num_rows($RSSuspension);
?>
   ...
   <blockquote>
     <blockquote>
      <blockquote>
       <blockquote>
         <blockquote>
           <blockquote> <font color="#FF6600" size="+1">Employee Suspension Form</font>
          
	  <?php include("Select_ID4Suspension.php");?>
      ...
      <table width="357" height="345" align="center" bgcolor="#EBEBEB">
        ...
        <td><textarea name="Suspension_Reason" id="Suspension_Reason" cols="30" rows="12"></textarea></td>
        </tr>
        <tr valign="baseline">
          <td nowrap="nowrap" align="right">Suspension Date:</td>
          <td><script type='text/JavaScript' src="../../Calender/scw.js" ></script>
		
		<input type="text" name="Suspension_Date" onclick='scwShow(this,event);' value='<?php echo date("Y-m-d");?>' /></td>
        </tr>
     
        <tr valign="baseline">
          <td nowrap="nowrap" align="right">&nbsp;</td>
          <td><input type="submit" value="Suspend" onclick="return confirm('Are you sure you want to Suspend this Employee? ')" /></td>
        </tr>
      </table>
      <p>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($RSSuspension);
?>
<script type="text/javascript">
var MenuBar1 = new Spry.Widget.MenuBar("MenuBar1", {imgDown:"SpryAssets/SpryMenuBarDownHover.gif", imgRight:"SpryAssets/SpryMenuBarRightHover.gif"});
</script>
