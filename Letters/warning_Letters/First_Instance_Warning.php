            ...
            <div id="mainContent">
                ...
                if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
                    ...
                    while ($rowDA = mysql_fetch_array($resultDA, MYSQL_ASSOC)) {
                        ...
                        if ($rowDA['ID'] == $_GET['ID']) {
                            ...
                            $data = array('ID' => $_GET['ID']
                                ...
                                , 'First_Inistance' => $_POST['First_Inistance']
                                , 'First_Inistance_Date' => $_POST['First_Inistance_Date']
                            );
                            $mydb->where(array('ID' => $_GET['ID']));
                            $ResultUPDT = $mydb->update('disciplinary_action', $data);
                            echo "<script type=\"text/javascript\">confirm('You already issued First Inistance Warning Letter  for {$_POST['FirstName']}  {$_POST['MiddelName']} in previous time which is less than warning letter expiry Day.')</script>";
                            if ($ResultUPDT)
                                echo "<script type=\"text/javascript\">alert('You have updated First Inistance Warning Letter  for {$_POST['FirstName']}  {$_POST['MiddelName']}  Successfully.')</script>";
                    ...
                    if ($count == 0) {
                        ...
                        if ($Result1)
                            echo "<script type=\"text/javascript\">alert('You have issued First Inistance Warning Letter  for {$_POST['FirstName']}  {$_POST['MiddelName']}  Successfully.')</script>";
                    }
                }
                ...
                ?>
                <h1 class="form_lable">
                    <?php echo $obj_lang->get('First Instance Warning Letter', $lang); ?>
                </h1>
                <?php
                $_GET['TableName'] = "employee_personal_record";

                $_GET['OpenPage'] = "First_Instance_Warning";
                ...
                ?>" readonly="readonly" size="20" />
                        ...
                        <tr>
                            <td align="right"><?php echo $obj_lang->get('Date', $lang); ?>:</td>
                            <td><input type="Date" name="First_Inistance_Date"  value='<?php echo date("Y-m-d"); ?>' /></td>
                        </tr>
                        <tr valign="baseline">
                            <td colspan="2">
                                <br/>
                                <?php echo $obj_lang->get('First Instance', $lang) . $obj_lang->get('Sample', $lang) . $obj_lang->get('Warning Letter', $lang); ?>
                                <br/>   
                                <hr/>
                                <br/>
                                <textarea name="First_Inistance" style="width:600px;height: 400px;">
                                    <?php require_once('Warning Letters Format/First_Instance_Warning_Letter.php'); ?>
                                </textarea>
                            </td>
                        </tr>
                        <tr valign="baseline">
                            ...
                            <td align="right">
                                ...
                                <input type="submit" value="<?php echo $obj_lang->get('Issue This Letter', $lang); ?>" onClick="return confirm('Are you sure you want to Issue This Letter for this Employee?')" /></td>
                        </tr>
                    </table>
