...
<html><head>
        ...
        <script language="javascript"> 
            ...
            slmg[0]='<font color="#03F"> <h3>Today\'s Salary Increment Notification </h3></font>' 
<?php
$d = date("Y-m-d");
...
$querycount1 = "select count(ID) as NoEmployee,ID,FirstName,MiddelName,Department,
        Date_employement,Salary,0.80 as This_year_Increment,
        Salary+0.8 as Increment from ThekeyHRMSDB.employee_personal_record 
        where  datediff(curdate(),date_Employement) >=365 and datediff(curdate(),date_Employement) < 395";


$resultcount1 = mysql_query($querycount1);

while ($rowcount1 = mysql_fetch_array($resultcount1, MYSQL_ASSOC)) {
    $firstYeartotal = $rowcount1['NoEmployee'];
}

$querycount2 = "select count(ID) as NoEmployee,ID,FirstName,MiddelName,Department,
        Date_employement,Salary,0.50 as This_year_Increment,
        Salary+0.50 as Increment from ThekeyHRMSDB.employee_personal_record 
        where  datediff(curdate(),date_Employement) >=730 and datediff(curdate(),date_Employement) < 760";

$resultcount2 = mysql_query($querycount2);

while ($rowcount2 = mysql_fetch_array($resultcount2, MYSQL_ASSOC)) {
    $secondYeartotal = $rowcount2['NoEmployee'];
}


$query = "select ID,FirstName,MiddelName,Department,
        Date_employement,Salary,0.80 as This_year_Increment,
        Salary+0.8 as Increment from ThekeyHRMSDB.employee_personal_record 
        where  datediff(curdate(),date_Employement) >=365 and datediff(curdate(),date_Employement) < 395
        Union ALL
        select ID,FirstName,MiddelName,Department,
        Date_employement,Salary,0.50 as This_year_Increment,
        Salary+0.50 as Increment from ThekeyHRMSDB.employee_personal_record 
        where  datediff(curdate(),date_Employement) >=730 and datediff(curdate(),date_Employement) < 760";

if ($firstYeartotal > 0 OR $secondYeartotal > 0) {
    $totalnumber = $firstYeartotal + $secondYeartotal;
    ...
    while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
        ...
        ?>
                    ...
                    //slmg[<?php //echo $i;   ?>]='<?php
        //echo "<font  color=\"#FF9900\" size=\"+1\" face=\"Times New Roman, Times, serif\">"."{$row['FirstName']}"." {$row['MiddelName']}"." {$row['LastName']}"."<br>";
        ...
        ?>';
                    ...
                    slmg[<?php echo $i; ?>]='<?php
        echo "<font  color=\"#FF9900\" size=\"+1\" face=\"Times New Roman, Times, serif\">" . " Total {$totalnumber} Emplyee available on this month Increment <br>";

        echo "<font color=\"#03F\" size=\"+0\" >" . "<a href=\"../Salary Increment Report/SalaryIncrementReport.php\" target=\"_blank\" >{$firstYeartotal} employee </a>has First Year Increment(0.80 Cent per day) and <a href=\"../Salary Increment Report/SalaryIncrementReport.php\" target=\"_blank\" >{$secondYeartotal} emplyee</a>  has Second Year Increment(0.50 Cent per day) on thier Daily Payment rate" . "<p> </p>";
        ?>';
