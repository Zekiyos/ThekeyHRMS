...
if(isset($_GET['ID']))
$query_RSRecruitment = "SELECT * FROM recruitment WHERE ID='".$_GET['ID']."'";
else
...
$_GET['OpenPage']="RecruitmentDisplay";
...
<table  cellpadding="0" align="center" border="1" bordercolor="#FF6600">  <tr>
  ...
  <?php do { ?>
    <tr>
      <td><a  target="_blank" href="RecruitmentUpdate.php?ID=<?php echo $row_RSRecruitment['ID']; ?>"  ><?php echo "<p>Update</p> </a>";?></a>
      
     <a  href="javascript: if (confirm('Are You Sure You want to Delete this Recruited Person record?')) { window.location.href='RecruitmentDelete.php?ID=<?php echo $row_RSRecruitment['ID']; ?>' } else { void('') }; "
         ><?php echo "Delete </a>";?></a> <br /></td>
      <td><?php echo $row_RSRecruitment['Employer']; ?></td>
      ...
      <td><?php echo $row_RSRecruitment['Present_Allowance']; ?></td>
    </tr>
...
<table width="331" border="0" align="center">
  <tr>
    <td><?php if ($pageNum_RSRecruitment > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RSRecruitment=%d%s", $currentPage, 0, $queryString_RSRecruitment); ?>"><img src="../../images/First.gif" /></a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_RSRecruitment > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RSRecruitment=%d%s", $currentPage, max(0, $pageNum_RSRecruitment - 1), $queryString_RSRecruitment); ?>"><img src="../../images/Previous.gif" /></a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_RSRecruitment < $totalPages_RSRecruitment) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_RSRecruitment=%d%s", $currentPage, min($totalPages_RSRecruitment, $pageNum_RSRecruitment + 1), $queryString_RSRecruitment); ?>"><img src="../../images/Next.gif" /></a>
        <?php } // Show if not last page ?></td>
    <td><?php if ($pageNum_RSRecruitment < $totalPages_RSRecruitment) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_RSRecruitment=%d%s", $currentPage, $totalPages_RSRecruitment, $queryString_RSRecruitment); ?>"><img src="../../images/Last.gif" /></a>
        <?php } // Show if not last page ?></td>
  </tr>
</table>
</body>
