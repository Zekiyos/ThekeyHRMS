            ...
            <table width="331" border="0" align="center">
                <tr>
                    <td><?php if ($pageNum_RSDemotionDisplay > 0) { // Show if not first page    ?>
                            <a href="<?php printf("%s?pageNum_RSDemotionDisplay=%d%s", $currentPage, 0, $queryString_RSDemotionDisplay); ?>"><img src="../Img/First.gif" /></a>
                        <?php } // Show if not first page  ?></td>
                    <td><?php if ($pageNum_RSDemotionDisplay > 0) { // Show if not first page   ?>
                            <a href="<?php printf("%s?pageNum_RSDemotionDisplay=%d%s", $currentPage, max(0, $pageNum_RSDemotionDisplay - 1), $queryString_RSDemotionDisplay); ?>"><img src="../Img/Previous.gif" /></a>
                        <?php } // Show if not first page  ?></td>
                    <td><?php if ($pageNum_RSDemotionDisplay < $totalPages_RSDemotionDisplay) { // Show if not last page   ?>
                            <a href="<?php printf("%s?pageNum_RSDemotionDisplay=%d%s", $currentPage, min($totalPages_RSDemotionDisplay, $pageNum_RSDemotionDisplay + 1), $queryString_RSDemotionDisplay); ?>"><img src="../Img/Next.gif" /></a>
                        <?php } // Show if not last page  ?></td>
                    <td><?php if ($pageNum_RSDemotionDisplay < $totalPages_RSDemotionDisplay) { // Show if not last page   ?>
                            <a href="<?php printf("%s?pageNum_RSDemotionDisplay=%d%s", $currentPage, $totalPages_RSDemotionDisplay, $queryString_RSDemotionDisplay); ?>"><img src="../Img/Last.gif" /></a>
                        <?php } // Show if not last page  ?></td>
                </tr>
            </table>
        </p>
