            ...
            <table width="325" border="0" align="center">
                <tr>
                    <td><?php if ($pageNum_RSPromotionDisplay > 0) { // Show if not first page    ?>
                            <a href="<?php printf("%s?pageNum_RSPromotionDisplay=%d%s", $currentPage, 0, $queryString_RSPromotionDisplay); ?>"><img src="../Img/First.gif" /></a>
                        <?php } // Show if not first page  ?></td>
                    <td><?php if ($pageNum_RSPromotionDisplay > 0) { // Show if not first page   ?>
                            <a href="<?php printf("%s?pageNum_RSPromotionDisplay=%d%s", $currentPage, max(0, $pageNum_RSPromotionDisplay - 1), $queryString_RSPromotionDisplay); ?>"><img src="../Img/Previous.gif" /></a>
                        <?php } // Show if not first page  ?></td>
                    <td><?php if ($pageNum_RSPromotionDisplay < $totalPages_RSPromotionDisplay) { // Show if not last page   ?>
                            <a href="<?php printf("%s?pageNum_RSPromotionDisplay=%d%s", $currentPage, min($totalPages_RSPromotionDisplay, $pageNum_RSPromotionDisplay + 1), $queryString_RSPromotionDisplay); ?>"><img src="../Img/Next.gif" /></a>
                        <?php } // Show if not last page  ?></td>
                    <td><?php if ($pageNum_RSPromotionDisplay < $totalPages_RSPromotionDisplay) { // Show if not last page   ?>
                            <a href="<?php printf("%s?pageNum_RSPromotionDisplay=%d%s", $currentPage, $totalPages_RSPromotionDisplay, $queryString_RSPromotionDisplay); ?>"><img src="../Img/Last.gif" /></a>
                        <?php } // Show if not last page  ?></td>
                </tr>
            </table>
        </p>
