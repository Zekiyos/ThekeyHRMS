...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        <table  cellpadding="0" align="center" border="1" bordercolor="#FF6600">
            ...
            <?php do { ?>
                <tr>
                    <td><a  target="_blank" href="../Leaves/Leave Database_Update/SickLeave_Update/SickLeaveUpdate.php?Auto_ID=<?php echo $row_RSSickLeaveUpdate['Auto_ID']; ?>"  ><?php echo "<p>Update</p> </a>"; ?></a>
        ...
        <table width="355" border="0" align="center">
            <tr>
                <td><?php if ($pageNum_RSSickLeaveUpdate > 0) { // Show if not first page    ?>
                        <a href="<?php printf("%s?pageNum_RSSickLeaveUpdate=%d%s", $currentPage, 0, $queryString_RSSickLeaveUpdate); ?>"><img src="../Img/First.gif" /></a>
                    <?php } // Show if not first page  ?></td>
                <td><?php if ($pageNum_RSSickLeaveUpdate > 0) { // Show if not first page   ?>
                        <a href="<?php printf("%s?pageNum_RSSickLeaveUpdate=%d%s", $currentPage, max(0, $pageNum_RSSickLeaveUpdate - 1), $queryString_RSSickLeaveUpdate); ?>"><img src="../Img/Previous.gif" /></a>
                    <?php } // Show if not first page  ?></td>
                <td><?php if ($pageNum_RSSickLeaveUpdate < $totalPages_RSSickLeaveUpdate) { // Show if not last page   ?>
                        <a href="<?php printf("%s?pageNum_RSSickLeaveUpdate=%d%s", $currentPage, min($totalPages_RSSickLeaveUpdate, $pageNum_RSSickLeaveUpdate + 1), $queryString_RSSickLeaveUpdate); ?>"><img src="../Img/Next.gif" /></a>
                    <?php } // Show if not last page  ?></td>
                <td><?php if ($pageNum_RSSickLeaveUpdate < $totalPages_RSSickLeaveUpdate) { // Show if not last page   ?>
                        <a href="<?php printf("%s?pageNum_RSSickLeaveUpdate=%d%s", $currentPage, $totalPages_RSSickLeaveUpdate, $queryString_RSSickLeaveUpdate); ?>"><img src="../Img/Last.gif" /></a>
                    <?php } // Show if not last page  ?></td>
            </tr>
        </table>
    </body>
