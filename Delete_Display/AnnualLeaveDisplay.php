...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        <table  cellpadding="0" align="center" border="1" bordercolor="#FF6600">
            ...
            <?php do { ?>
                <tr>
                    <td><a  target="_blank" href="../Leaves/Leave Database_Update/AnnualLeave_Update/AnnualLeaveUpdate.php?Auto_ID=<?php echo $row_RSAnnualLeaveDisplay['Auto_ID']; ?>"  ><?php echo "<p>Update</p> </a>"; ?></a>
        ...
        <table width="340" border="0" align="center">
            <tr>
                <td><?php if ($pageNum_RSAnnualLeaveDisplay > 0) { // Show if not first page    ?>
                        <a href="<?php printf("%s?pageNum_RSAnnualLeaveDisplay=%d%s", $currentPage, 0, $queryString_RSAnnualLeaveDisplay); ?>"><img src="../Img/First.gif" /></a>
                    <?php } // Show if not first page  ?></td>
                <td><?php if ($pageNum_RSAnnualLeaveDisplay > 0) { // Show if not first page   ?>
                        <a href="<?php printf("%s?pageNum_RSAnnualLeaveDisplay=%d%s", $currentPage, max(0, $pageNum_RSAnnualLeaveDisplay - 1), $queryString_RSAnnualLeaveDisplay); ?>"><img src="../Img/Previous.gif" /></a>
                    <?php } // Show if not first page  ?></td>
                <td><?php if ($pageNum_RSAnnualLeaveDisplay < $totalPages_RSAnnualLeaveDisplay) { // Show if not last page   ?>
                        <a href="<?php printf("%s?pageNum_RSAnnualLeaveDisplay=%d%s", $currentPage, min($totalPages_RSAnnualLeaveDisplay, $pageNum_RSAnnualLeaveDisplay + 1), $queryString_RSAnnualLeaveDisplay); ?>"><img src="../Img/Next.gif" /></a>
                    <?php } // Show if not last page  ?></td>
                <td><?php if ($pageNum_RSAnnualLeaveDisplay < $totalPages_RSAnnualLeaveDisplay) { // Show if not last page   ?>
                        <a href="<?php printf("%s?pageNum_RSAnnualLeaveDisplay=%d%s", $currentPage, $totalPages_RSAnnualLeaveDisplay, $queryString_RSAnnualLeaveDisplay); ?>"><img src="../Img/Last.gif" /></a>
                    <?php } // Show if not last page  ?></td>
            </tr>
        </table>
    </body>
