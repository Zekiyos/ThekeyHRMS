        ...
        <div id="thekey_page">
            ...
            <div id="mainContent"><!-- InstanceBeginEditable name="MainContent" -->
                ...
                if (isset($_GET['Auto_ID']) && (!isset($_POST['MM_update']))) {
                    ...
                    $result = $mydb->delete('health_care_insurance_Definition');
                    if ($result) {
                ...
                $mydb = new DataBase();
                $header_lable = 'Health Care Insurance Definition';


                $mydb = filter($mydb, true);
                $result = $mydb->get('health_care_insurance_Definition');
                $total_num_row = $result['total_row'];                 $result = $result['result'];

                $header_info = $mydb->show_column('health_care_insurance_Definition');
                ksort($header_info);

                $update_page = 'HealthCareInsuranceDefinitionUpdate.php';
                $key_filed = 'Auto_ID';

                $hide = array('Auto_ID' => 'Auto_ID');
                $table = "health_care_insurance_Definition";
