...
//                }
        ...
        /* if($Working_Hour != $Working_Hour_Hold){

          $WorkingHR=round($this->hour2sec($WorkingHR) * $this->hour2sec($Working_Hour_Hold)/
          $this->hour2sec($Working_Hour));

          $WorkingHR = $this->sec2hour($WorkingHR);
...
//************** Day OT Allocation**********************//
    ...
    public function get_offday($ID, $Date) {
        ...
        if (mysql_num_rows(mysql_query($sqlOffDay))) {


            $resultOT = mysql_query($sqlOffDay);
            $rowOT = mysql_fetch_array($resultOT, MYSQL_ASSOC);
 ...
 where ID='" . $ID . "' and Date='" . $Date . "'"; // AND Department='" . $Department . "'";
    ...
    function generate_attendance_allocation($Scan_ID, $Scan_Department, $Scan_Date, $Scan_Start, $Scan_Start_Break, $Scan_End_Break, $Scan_End, $operation = NULL) {
        ...
        if ((($Scan_Date == '2012-10-16') or ($Scan_Date == '2012-10-17'))) {
            if (trim(strtoupper(substr($Scan_Department, -5, 5))) == trim(strtoupper('Spray'))) {
                $Scan_Start = "14:00:00";
            }
        }
    ...
    public function Attendance_Summary($TableName, $Department, $From_Date, $To_Date) {
        ...
        if ($TableName == 'Attendance_Allocation') {
            ...
            if (!$sqlAL) {
                ...
                $sqlAL = $sqlAL . " 
                WHERE (" . $dip_filter . ")  and `Date`>= '" . $From_Date . "'  and `Date`<='" . $To_Date . "'
