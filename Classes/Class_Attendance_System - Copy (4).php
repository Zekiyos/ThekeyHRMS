...
class Attendance_System extends connection {
 public function getEmployeeTerminatedDate($ID) { $queryOT = "SELECT ID,Terminated_Date FROM terminated_employee Where ID='" . $ID . "'"; $resultOT = mysql_query($queryOT); $rowOT = mysql_fetch_array($resultOT); if (mysql_num_rows($resultOT)) { return $rowOT['Terminated_Date']; }else return False; }
...
//        $totalsec = $to_time + $from_time;
    ...
    public function OT($scanData, $workingTimeDefinition, $OTDefinition, $round = NULL) {
        ...
        /*         * *******coldroom('GH 11-12 Cold Room') case //if only breaks are not avaial and scan data registed Scan_End in the in Start_Break **************** */
        if (strtoupper(substr(trim($Scan_Department), -9, 9)) == trim(strtoupper('Cold Room'))) {


            if (($Scan_Start_Break != NULL) and ($Scan_End_Break != NULL) and ($Scan_End != NULL)) {

                //$DayOTHR = $this->time_diff($Scan_End_Break, $Scan_Start_Break);
				 $DayOTHR =$this->time_diff($Scan_End,$Scan_End_Break);

				 if ((trim(strtoupper($NightOT)) == "Y") and ($Scan_Start_Break > $NightOT_Start)) {
                    $NightOTHR = $this->time_diff($Scan_Start_Break, $NightOT_Start);
                }
            }
        }
...
//                    echo '<hr>';
 ...
 where ID='" . $ID . "' and Date='" . $Date . "'"; // AND Department='" . $Department . "'";
  ...
  $terminatedDate = $this->getEmployeeTerminatedDate($ID);


        if (($Holday_Name != trim(strtoupper("Not Holyday"))) && (( $Scan_Date < $terminatedDate) || ($terminatedDate == False))) {
            $Status = "Holy Day";
            $WorkingHR = $Working_Hour_Hold;
        }

		if (($Status == "Holy Day") and ($HolydayOTHR > "00:00:00") and ($HolydayOTHR < "08:00:00"))
		$HolydayOTHR ='08:00:00';
...
AND `To_Date` >='" . $Date . "'and UPPER(`Day_Name`)='" . trim(strtoupper(date("l", strtotime($Date)))) . "'";
	...
	public function showAttendanceAllocation($fromDate, $toDate, $Department, $quickFind = NULL) {
        $sql = "SELECT * FROM Attendance_Allocation a 
        ...
        if (is_array($Department)) {
            ...
            $departmentFilter = "e.Department='" . $Department . "'";
        }
...
@Total_Absent_Hour:=(Sum(Time_To_Sec(if(isnull(attendance_allocation.AbsentHR),'00:00:00',attendance_allocation.AbsentHR))) /3600) AS Total_Absent_Hour 
  ...
  From Attendance_Allocation WHERE `Status` LIKE '%Leave%' AND Status NOT LIKE '%Special Leave Without Payment%' AND ID='" . $ID . "' and attendance_allocation.Date>= '" . $fromDate . "'  and attendance_allocation.Date<='" . $toDate . "' Group By ID";
        $result = mysql_query($sql);
//echo '<hr>'.$sql;
        $maxWorkingDay = 26;
...
//		echo "<td>" . $rowAS['From_Date'] . "</td>";
   ...
   public function generateAttendanceSummary($Department = 'json', $From_Date, $To_Date, $summarize = FALSE) {
    ...
    session_start();
           ...
           if ($summarize) {
            ...
            if (isset($_POST['selected_Department']))
                $Department = json_decode($_POST['selected_Department']);
            if (isset($_POST['selected_monthYear'])) {
            $yearMonth = $_POST['selected_monthYear'];
        } else {
            $d= (int)substr($From_Date, -1, 2);
            $m=(int)substr($From_Date, 5, 2);
            $y=(int)substr($From_Date, 0, 4);
            $yearMonth_date = mktime(0, 0, 0,$m ,$d, $y);
        ...
        $sql = "SELECT *,e.Department AS Department,CAST((SUBSTRING_INDEX(e.`ID`, '-', -1))AS UNSIGNED) AS IDNO FROM employee_personal_record e JOIN Department ON e.Department=Department.Department
  LEFT JOIN `probation_period_terminated_employee` PT ON e.ID=PT.ID
  INNER JOIN User_Attendance ON Department.Section = User_Attendance.`Department` WHERE Date_Employement<='" . $Attendance_Closing_Date . "'";
        ...
        $sql = $sql . "  AND (" . $dip_filter . ") Order By e.Department,IDNO";
...
<tbody>";
        ...
        while ($rowAS = mysql_fetch_assoc($result, MYSQL_ASSOC)) {
            ...
            $totalSumDays = $workingDay + $totalLeaveDay + $totalAbsentDay;
          ...
          //echo '<hr>2'.$ID.'--'.$workingDay;                
			...
			//date('m',$Attendance_Opening_Date)
			   ...
			   if(($rowAS['Date_Employement'] < $Attendance_Opening_Date) && ($rowAS['Terminated_Date']== NULL)){
             $diff1 = 26-$totalSumDays;
            ...
            /* additional 10 hour OT for Harvesting, Line Manger,Vocom Operator,Transport Irrigation and Spray Department
                                    if (((strtoupper(substr(trim( $rowAS['Department']), -10)) == 'HARVESTING')
                                            || (strtoupper(substr(trim( $rowAS['Department']), -12)) == 'LINE MANAGER')
                                          || (strtoupper(substr(trim( $rowAS['Department']), -23)) == 'IRRIGATION AND PUSHCART')
                                          || (strtoupper(substr(trim( $rowAS['Department']), -5)) == 'SPRAY')
                                            ) 
                                          // &&(strtoupper($yearMonth)=='2016_NOVEMBER') 
										  && ($rowAS['Date_Employement'] < '2016-10-30')
										  && ($workingDay > 0)
										   ) {
                                        $totalDayOTHour=$totalDayOTHour+10;
                                    }
*/
			...
			if ($workingDay < 0) {
			    $totalLeaveDay = $workingDay +  $totalLeaveDay;
                $workingDay = 0;
			 ...
			 if (($summarize)&&( $totalDays>0)&&($rowAS['Date_Employement'] <=$Attendance_Closing_Date )) {
			  ...
			  $sqlchk = "SELECT * FROM Biometric_Attendance_Summary 
                ...
                $result_chk=mysql_query($sqlchk);
				if(mysql_num_rows($result_chk)){
				echo "Attendance Summary is alreday Confirmed for Payroll Output";
				return FALSE;
				}
				
				
                $sqldel = "DELETE FROM Biometric_Attendance_Summary 
                    WHERE `Year_Month`='" . $yearMonth . "' AND ID='" . $ID . "'";
                mysql_query($sqldel);

                $sqlInsert = "INSERT INTO Biometric_Attendance_Summary (`ID`, `FirstName`, `MiddelName`, `LastName`, `Department`, `Working_Day`, `Leave_Day`,  `Absent_Day`, `DayOT_Hour`, `NightOT_Hour`, `OffDayOT_Hour`, `HolyDayOT_Hour`, `Year_Month`, `Prepared`, `Department_Manager`, `Checked`, `Approved`,`ModifiedBy`)";
                $sqlInsert.="values ('" . $ID . "','" . $rowAS['FirstName'] . "','" . $rowAS['MiddelName'] . "','" . $rowAS['LastName'] . "','" .
                        $rowAS['Department'] . "','" . $workingDay . "','" . $totalLeaveDay . "','" . $totalAbsentDay . "','" . $totalDayOTHour . "','" . $totalNightOTHour . "','" . $totalOffDayOTHour . "','" . $totalHolyDayOTHour . "','" . $yearMonth . "','" . $rowAS['Prepared'] . "','" . $rowAS['Department_Manager'] . "','" . $rowAS['Checked'] . "','" . $rowAS['Approved'] . "','".$_SESSION['MM_Fullname']."')";
                
				
				
				$sqlInsert1 = "INSERT INTO thekey_bas (`ID`, `FirstName`, `MiddelName`, `LastName`, `Department`, `Working_Day`, `Leave_Day`,  `Absent_Day`, `DayOT_Hour`, `NightOT_Hour`, `OffDayOT_Hour`, `HolyDayOT_Hour`, `Year_Month`, `Prepared`, `Department_Manager`, `Checked`, `Approved`,`ModifiedBy`,`time`)";
                $sqlInsert1.="values ('" . $ID . "','" . $rowAS['FirstName'] . "','" . $rowAS['MiddelName'] . "','" . $rowAS['LastName'] . "','" .
                        $rowAS['Department'] . "','" . $workingDay . "','" . $totalLeaveDay . "','" . $totalAbsentDay . "','" . $totalDayOTHour . "','" . $totalNightOTHour . "','" . $totalOffDayOTHour . "','" . $totalHolyDayOTHour . "','" . $yearMonth . "','" . $rowAS['Prepared'] . "','" . $rowAS['Department_Manager'] . "','" . $rowAS['Checked'] . "','" . $rowAS['Approved'] ."','".$_SESSION['MM_Fullname']."','" . time()."')";
                //echo $sqlInsert1;
				$resultInsert1 = mysql_query($sqlInsert1);
           ...
           //echo "<td>" . $totalNoDays . "</td>";
            ...
            if ($count == $numrows) {
              ...
              /*  echo '<td  colspan="3" ><b>Prepared By: ' . $rowAS['Prepared'] . '</b></td>';
               ...
               // echo '<tr><td colspan="35" align="right"><img  src="../images/thekeysoft.JPG"  height="25" width="80" /></td></tr>';
        ...
        if (isset($resultInsert) && ($resultInsert) && ($summarize)) {
            $alert_message = 'Attendance Summarized for '.$_POST['selected_Department'].' department '.$yearMonth.' month Successfully!';
            $alertType = 'Success';
        } else if (($summarize)) {
            $alert_message = 'Attendance Summarized is failed';
            $alertType = 'ActionFail';
        }
        if (($summarize)){
            echo $alert_message;
            return;
    ...
    public function generateAttendanceSummaryXX($Department, $From_Date, $To_Date) {
...
<tbody>";
    ...
    public function displayAttendanceSummaryX($Department, $yearMonth) {
...
<tbody>";
   ...
   public function array_Summmary($array) {
        $newarr = array();

        if (is_array($array)) {
//            print_r($array);
            foreach ($array as $value) {
                foreach ($value as $key => $secondValue1) {
                    if (!isset($newarr[$key])) {
                        $newarr[$key] = 0;
                    }
					$secondValue=(float)$secondValue1;
                    if (is_numeric($secondValue)) {
                        $newarr[$key]+=$secondValue;
                    } else
                    if (($key == 'Section') || ($key == 'Department'))
                        $newarr[$key] = $secondValue; //'-';
                    else
                    if (($key == 'ID'))
                        $newarr[$key] = count($array);
                    else
                    if (($key == 'FirstName'))
                        $newarr[$key] = $value['Department'];
                    else
                        $newarr[$key] = '-';
                }
            }
    ...
    public function displayAttendanceSummary($Department, $yearMonth) {
        ...
        $sql = "SELECT CAST((SUBSTRING_INDEX(Biometric_Attendance_Summary.ID, '-', -1))AS UNSIGNED) AS IDNO,Biometric_Attendance_Summary.*
		FROM Biometric_Attendance_Summary 
		JOIN employee_personal_record ON employee_personal_record.ID=Biometric_Attendance_Summary.ID 
                    WHERE `Year_Month`='" . $yearMonth . "' ";
...
date_default_timezone_set('Africa/Nairobi');
  ...
  INNER JOIN User_Attendance ON Department.Section = User_Attendance.`Department` WHERE Date_Employement<='" . $Attendance_Closing_Date . "'";
        ...
        while ($rowAS = mysql_fetch_assoc($result, MYSQL_ASSOC)) {
            ...
            $sql3 = "INSERT INTO Biometric_Attendance_Summary (`ID`, `FirstName`, `MiddelName`, `LastName`, `Department`, `Working_Day`, `Leave_Day`,  `Absent_Day`, `DayOT_Hour`, `NightOT_Hour`, `OffDayOT_Hour`, `HolyDayOT_Hour`, `Year_Month`, `Prepared`, `Department_Manager`, `Checked`, `Approved`)";
