			...
			if ($workingDay < 0) {
			 ...
			 if (($summarize)&&( $totalDays>0)) {
                ...
                $sqlInsert1.="values ('" . $ID . "','" . $rowAS['FirstName'] . "','" . $rowAS['MiddelName'] . "','" . $rowAS['LastName'] . "','" .
                        $rowAS['Department'] . "','" . $workingDay . "','" . $totalLeaveDay . "','" . $totalAbsentDay . "','" . $totalDayOTHour . "','" . $totalNightOTHour . "','" . $totalOffDayOTHour . "','" . $totalHolyDayOTHour . "','" . $yearMonth . "','" . $rowAS['Prepared'] . "','" . $rowAS['Department_Manager'] . "','" . $rowAS['Checked'] . "','" . $rowAS['Approved'] ."','','" . time()."')";
                //echo $sqlInsert1;
...
//            print_r($array);
    ...
    public function displayAttendanceSummary($Department, $yearMonth) {
		...
		FROM Biometric_Attendance_Summary  JOIN employee_personal_record ON employee_personal_record.ID=Biometric_Attendance_Summary.ID 
                    WHERE Month_Year='" . $yearMonth . "' ";

        $dip_filter = '';
        if (is_array($Department)) {
            foreach ($Department as $dep_key => $dep_value) {
                $filter = "Biometric_Attendance_Summary.`Department`='" . $dep_value . "'";
                if (!$dip_filter == '') {
                    $dip_filter .= ' or ' . $filter;
                } else {
                    $dip_filter = $filter;
            ...
            $dip_filter = "Biometric_Attendance_Summary.`Department`='" . $Department . "'";
        }
            ...
            if ($count == $numrows) {
                ...
                foreach ($arrayAtendanceSummary as $key => $value) {
                    ...
                    if (!in_array(trim($key), $not_sumable)) { //displaying the total result in report field layout
                        $value = trim($value);
                        if (is_numeric($value))
                            echo '<td class="right">' . number_format($value) . '</td>';
                        else
                        if (($key != 'Department'))
