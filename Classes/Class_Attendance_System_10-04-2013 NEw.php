...
//// filename for download
 ...
 Order By attendance_allocation.Department,IDNO";
    ...
    public function Attendance_Scan_Sheet_Importer($Department, $From_Date, $To_Date) {
        ...
        $scanhostName = "192.168.1.200";
        $scanuserName = "root";
        $scanpassword = "EWSadmin";
        $scandbName = "Identysoft";

        $scanCon = mysql_pconnect($scanhostName, $scanuserName, $scanpassword) or trigger_error(mysql_error(), E_USER_ERROR);

        mysql_select_db($scandbName);

        mysql_set_charset('UTF8', $scanCon);

        $scanSheetSql = "SELECT Waarde AS ID,`user`.`Voornaam` AS FirstName,
        ...
        $scanSheetSql .=" and s.Datum>='" . $From_Date . "'";
        $scanSheetSql .=" and  s.Datum<='" . $To_Date . "'";
        $scanSheetSql .= " AND (";
        ...
        if (is_array($Department)) {
            foreach ($Department as $key => $value) {
                if ($first) {
                    $scanSheetSql .=' gg.`Omschrijving` LIKE \'%' . $value . '%\'';
                    $first = false;
                }
                else
                    $scanSheetSql .=' or gg.`Omschrijving` LIKE \'%' . $value . '%\'';
            }
        }
        else {
            $scanSheetSql .=' gg.`Omschrijving` LIKE \'%' . $Department . '%\'';
        }
        ...
        $scanResult = mysql_query($scanSheetSql, $scanCon);


        $numrows = mysql_numrows($scanResult);
		

        while ($scanRow = mysql_fetch_array($scanResult, MYSQL_ASSOC)) {

            $arrayScanRow[] = $scanRow;
        }
        ...
        $hrmshostName = "192.168.1.201";
        $hrmsuserName = "root";
        $hrmspassword = "EWSadmin";
        $hrmsdbName = "ThekeyHRMSDB";

        $hrmsCon = mysql_pconnect($hrmshostName, $hrmsuserName, $hrmspassword) or trigger_error(mysql_error(), E_USER_ERROR);

        foreach ($arrayScanRow as $arrayScanRowkey => $arrayScanRowValue) {
           ...
           (`ID`, `FirstName`, `MiddelName`, `LastName`, `Department`, `Date`,
            ...
            ";
            $sql.="('" . $arrayScanRowValue['ID'] . "','" . $arrayScanRowValue['FirstName'] . "','" . $arrayScanRowValue['MiddelName'] . "','" .
                    $arrayScanRowValue['LastName'] . "','" . $arrayScanRowValue['Department'] . "','" . $arrayScanRowValue['Date'] . "','" .
                    $arrayScanRowValue['Start'] . "','" . $arrayScanRowValue['Start_Break'] . "','" . $arrayScanRowValue['End_Break'] . "','" . $arrayScanRowValue['End'] . "')";

					
					
            /*             * ***************************
              $sql.='('.$scanIDValue.','. $firstNameValue.','. $middelNameValue.','.
              $lastNameValue.','. $departmentValue.','.$dateValue.','.
              $startScanValue.','. $startBreakScanValue.',' .$endBreakScanValue.','. $endScanValue.')';
             * ***************
             */

            $resultAS = mysql_query($sql,$hrmsCon);
        }
