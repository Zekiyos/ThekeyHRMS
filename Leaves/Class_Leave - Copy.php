...
FROM `annual_leave`
 WHERE ID='" . $ID . "' GROUP BY ID";
    ...
    public function ALYearAllocation($totalALdays, $initialALdays, $dateofemployeement) {
        ...
        echo "<br><font color=\"#0000FF\" face=\"Times New Roman, Times, serif\" size=\"+1\">?? ???? ???/Yearly Allocation for Annual Leave  <font color=\"#FF0000\">" . $totalALdays . "</font> ??? / days</br>";
        //assign the total for the current ccause the total is lessthan the initial
        if ($totalALdays <= $initialALdays) {
            echo "Year $currentyear - $currentmonth :<font color=\"#FF0000\">" . $totalALdays . "</font>" . " ??? / days</font></br>";
        } else
        if ($totalALdays > $initialALdays) {
            ...
            if ($CurrentYearAL < $totalALdays) {
                ...
                echo "Year $lastyear - $DEmonth to Year $currentyear - $currentmonth <font color=\"#FF0000\">" . $CurrentYearALDays . "</font>" . " ??? / days</br>";
                ...
                for ($j = $CurrentYearAL; $j <= $totalALdays; $j = $CurrentYearAL--) {
                    ...
                    echo "Year $prvyear - $DEmonth  to Year $lastyear - $DEmonth <font color=\"#FF0000\">" . $CurrentYearAL . "</font> ??? / days<br/>";
                ...
                if (isset($prvyear))
                    ...
                    $prvyear = $lastyear;
                echo "Year $DEyear - $DEmonth  to Year $prvyear - $DEmonth <font color=\"#FF0000\">" . $totalALdays . "</font> ??? / days</font><br/>";
            }
        }
    }
