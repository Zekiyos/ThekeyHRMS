...
//$query_RSSalaryIncrement = "select ID,FirstName,MiddelName,Department, Date_employement,Salary,0.80 as This_year_Increment, Salary+0.8 as Increment from aqhrmsdb.employee_personal_record  where  datediff(curdate(),date_Employement) >=365 and datediff(curdate(),date_Employement) < 395 Union ALL select ID,FirstName,MiddelName,Department, Date_employement,Salary,0.50 as This_year_Increment, Salary+0.50 as Increment from aqhrmsdb.employee_personal_record  where  datediff(curdate(),date_Employement) >=730 and datediff(curdate(),date_Employement) < 760";
...
IF(Salary=564,591,IF(Salary=740,765,If(Salary=726,755,if(Salary=681,705,Salary)))) as After_Increment from aqhrmsdb.employee_personal_record 
...
IF(Salary=591,15,IF(Salary=765,25,If(Salary=755,725,if(Salary=705,24,0)))) as This_year_Increment,IF(Salary=591,606,IF(Salary=765,790,If(Salary=755,780,if(Salary=705,729,Salary)))) as After_Increment from aqhrmsdb.employee_personal_record  
...
function PrintContent()
    {
		 if (confirm('Are you sure you want to Print Out the Search result in current setting of paper scale?')) {
			 var DocumentContainer = document.getElementById('SalaryIncrement');
