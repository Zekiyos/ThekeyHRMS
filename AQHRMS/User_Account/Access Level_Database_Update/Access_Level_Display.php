...
<table width="559" border="0" align="center">
  <tr>
    <td><?php if ($pageNum_RSAccessLevel > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RSAccessLevel=%d%s", $currentPage, 0, $queryString_RSAccessLevel); ?>"><img src="First.gif" /></a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_RSAccessLevel > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RSAccessLevel=%d%s", $currentPage, max(0, $pageNum_RSAccessLevel - 1), $queryString_RSAccessLevel); ?>"><img src="Previous.gif" /></a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_RSAccessLevel < $totalPages_RSAccessLevel) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_RSAccessLevel=%d%s", $currentPage, min($totalPages_RSAccessLevel, $pageNum_RSAccessLevel + 1), $queryString_RSAccessLevel); ?>"><img src="Next.gif" /></a>
        <?php } // Show if not last page ?></td>
    <td><?php if ($pageNum_RSAccessLevel < $totalPages_RSAccessLevel) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_RSAccessLevel=%d%s", $currentPage, $totalPages_RSAccessLevel, $queryString_RSAccessLevel); ?>"><img src="Last.gif" /></a>
        <?php } // Show if not last page ?></td>
  </tr>
</table>
<p>&nbsp;</p>
...
<li><a href="../../Letters/warning Letters/Disciplinary Action Database Update/TerminationDisplay.php" title="Termination Data" target="_blank"><?php echo $obj_lang->get('Termination Data', $lang); ?></a></li>
      ...
      <li><a href=""><?php echo $obj_lang->get('Access Level', $lang); ?></a>
            ...
            <li><a href="Access_Level_Display.php"><?php echo $obj_lang->get('Access Level Data ...', $lang); ?></a></li>            
