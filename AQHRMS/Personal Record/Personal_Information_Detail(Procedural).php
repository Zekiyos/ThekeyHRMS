...
function toggle_all() {
	...
	document.getElementById('Discipline').style.display = (document.getElementById('Discipline').style.display == "none") ? "" : "none";
...
<em>was born 
  ...
  as
    <?php echo "{$row['Position']}";
				...
				echo "<br>Education Status ".$row['Educational_Status']."";
					}
	...
	if ( $sex="Male"){$he="he";} else {$he="she";}
       			 		$queryAL  = "SELECT * FROM Annual_leave_Calculate";
				...
				while($rowAL = mysql_fetch_array($resultAL, MYSQL_ASSOC))
				{
					...
					if ($rowAL['ID'] == $_GET['ID'])
					{
						echo "<br>Experince In AQ Roses: ".$he." works for"." {$rowAL['WorkingMonth']}"." months"."("." {$rowAL['WorkingYear']}"." Years)<br>";
						echo "<div id=\"Leave\" style=\"display: none;\">";			
					echo "<blockquote><blockquote><blockquote><u><b>Leave days </u></b></blockquote></blockquote></blockquote>";					
					echo "<li type=>Last annual leave ".$rowAL['FirstName']." took on"." {$rowAL['Calculated_Date']}"."<br>";	
					
					echo "<li>Total Number of annnual leave ".$rowAL['FirstName']." left:"." {$rowAL['TotalLeftDay']}"."<br>";		
					echo "<blockquote><li>from this Year". " {$rowAL['ThisYearLeft']}"."<br>";
					echo "<li>from Last"." {$rowAL['LastYearLeft']}"."<br>";
					echo "<li>from Before Last"." {$rowAL['BeforeLastYearLeft']}"."<br></blockquote>";
				...
				?>
                 <?php 
       			 		...
       			 		$queryWL  = "SELECT * FROM paternity_leave";
                ...
                <?php 
       			 		...
       			 		$queryWL  = "SELECT * FROM Special_leave";
						$resultWL = mysql_query($queryWL);
				while($rowWL = mysql_fetch_array($resultWL, MYSQL_ASSOC))
				{
					...
					if ($rowWL['ID'] == $_GET['ID'])
					{
						echo "<br> <li>Number Of Special leave " .$rowWL['FirstName']." tooks" ;
						echo " {$rowWL['SpecialLeaveDays']} ";
						echo "on"." {$rowWL['SpecialLeave_Taken_Date']}";
				...
				while($row = mysql_fetch_array($result, MYSQL_ASSOC))
				{
					...
					echo "<div id=\"Equipment\" style=\"display: none;\">";
				...
				while($row = mysql_fetch_array($result, MYSQL_ASSOC))
				{
					...
					echo "<div id=\"Training\" style=\"display: none;\">";
               ...
               <?php 
       			 		...
       			 		if(isset($_GET['ID'] )){	
						$queryJB ="SELECT `employee_personal_record`.ID,Job_Description,Job_Description_Amharic FROM `contract_letter` JOIN `employee_personal_record` ON Contract_letter.Department =`employee_personal_record`.Department";
				//WHERE `employee_personal_record`.ID = ".$_GET['ID']."";
						
						$resultJB = mysql_query($queryJB);
					echo "<div id=\"Job_Description\" style=\"display: none;\">";	
