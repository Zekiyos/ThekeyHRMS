...
<table width="406" height="44" border="0" align="center">
  <tr>
    <td><?php if ($pageNum_RSDepartmentDisplay > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RSDepartmentDisplay=%d%s", $currentPage, 0, $queryString_RSDepartmentDisplay); ?>"><img src="First.gif" /></a>
      <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_RSDepartmentDisplay > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RSDepartmentDisplay=%d%s", $currentPage, max(0, $pageNum_RSDepartmentDisplay - 1), $queryString_RSDepartmentDisplay); ?>"><img src="Previous.gif" /></a>
      <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_RSDepartmentDisplay < $totalPages_RSDepartmentDisplay) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_RSDepartmentDisplay=%d%s", $currentPage, min($totalPages_RSDepartmentDisplay, $pageNum_RSDepartmentDisplay + 1), $queryString_RSDepartmentDisplay); ?>"><img src="Next.gif" /></a>
      <?php } // Show if not last page ?></td>
    <td><?php if ($pageNum_RSDepartmentDisplay < $totalPages_RSDepartmentDisplay) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_RSDepartmentDisplay=%d%s", $currentPage, $totalPages_RSDepartmentDisplay, $queryString_RSDepartmentDisplay); ?>"><img src="Last.gif" /></a>
      <?php } // Show if not last page ?></td>
  </tr>
</table>
</p>
