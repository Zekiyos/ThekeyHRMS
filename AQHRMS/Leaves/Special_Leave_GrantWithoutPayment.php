...
/*	
$UnpaidHourSpecialLeave="Select @TotalMonthOffDay:=DAY( LAST_DAY('".$_POST['SpecialLeave_Taken_Date']."' ))/7 AS TotalMonthOffDay,
									@AbsentDay:= ".$_POST['SpecialLeaveDays']." AS AbsentDay,
									@AbsentOffDay:=@AbsentDay /7 AS AbsentOffDay,
									@AbsentWorkingDay:=@AbsentDay - @AbsentOffDay AS AbsentWorkingDay,
									@AbsentWorkingHour:=@AbsentWorkingDay*8 AS AbsentWorkingHour";
							
						$resultUnpaidHourSpecialLeave = mysql_query($UnpaidHourSpecialLeave);
						
				
						
						
				while($rowUnpaidHourSpecialLeave = mysql_fetch_array($resultUnpaidHourSpecialLeave, MYSQL_ASSOC))
				{
				//$AbsentWorkingHourSpecialLeave =$rowUnpaidHourSpecialLeave['AbsentWorkingHour'];
*/						
				$AbsentWorkingHourSpecialLeave =$_POST['SpecialLeaveDays'];
				
				$sqlupdateAbsentHour= "UPDATE `aqhrmsdb`.`week_6` SET 
										`No_Absent`=(
   Select `No_Absent` FROM (
           SELECT `No_Absent` FROM week_6 WHERE  ID='".$_GET['ID']."') AS Sql1 ) + ".$AbsentWorkingHourSpecialLeave ." WHERE  ID='".$_GET['ID']."'";
...
//				}
  ...
  $insertSQL = sprintf("INSERT INTO Special_leave (ID, FirstName, MiddelName, LastName,Department, SpecialLeaveDays, SpecialLeave_Taken_Date, ReportOn,LeaveType) VALUES (%s,%s, %s, %s, %s,%s, %s, %s, %s)",
                       GetSQLValueString($_POST['ID'], "text"),
                       GetSQLValueString($_POST['FirstName'], "text"),
                       GetSQLValueString($_POST['MiddelName'], "text"),
                       GetSQLValueString($_POST['LastName'], "text"),
					   GetSQLValueString($_POST['Department'], "text"),
                       GetSQLValueString($_POST['SpecialLeaveDays'], "int"),
                       GetSQLValueString($_POST['SpecialLeave_Taken_Date'], "date"),
                       GetSQLValueString($_POST['ReportOn'], "date"),
					   GetSQLValueString($LeaveType, "text"));

  mysql_select_db($database_HRMS, $HRMS);
  $Result1 = mysql_query($insertSQL, $HRMS) or die(mysql_error());

	echo "<script type=\"text/javascript\">alert('You have Granted the Special Leave form the employee  Seccussfully.')</script>";
	}
//  $insertGoTo = "../test.htm";
 // if (isset($_SERVER['QUERY_STRING'])) {
  ...
  <div id="mainContent"><!-- InstanceBeginEditable name="MainContent" -->
        ...
        <blockquote>
          <blockquote>
            <blockquote>
              <blockquote>
			   <?php //include("Select_ID4SpecialLeaveGrantWithoutPayment.php");?>
               <font color="#FF6600" size="+1"><?php  echo $obj_lang->get('Special Leave Grant', $lang); ?></font> <font color="#FF0000" size="+1"><?php  echo $obj_lang->get('(WITHOUT PAYMENT)', $lang); ?> </font>
               <?php
      ...
      <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1" onmouseover="Numberofdays()">
        <table align="center" bgcolor="#EBEBEB">
          ...
          <tr valign="baseline">
            ...
            <td><input type="text" id="SpecialLeaveDays" name="SpecialLeaveDays" value="" size="4" readonly="readonly" /></td>
          </tr>
          <tr valign="baseline">
            <td height="34" align="right" nowrap="nowrap"><?php  echo $obj_lang->get('Special Leave Taken Date', $lang); ?>:</td>
            <td><input type="text" id="SpecialLeave_Taken_Date"  name="SpecialLeave_Taken_Date" onclick='scwShow(this,event);' value="<?php echo date("Y-m-d"); ?>" size="15"  /></td>
          </tr>
          <tr valign="baseline">
		...
		<input type="text" id="ReportOn" name="ReportOn" onclick='scwShow(this,event);' value='<?php echo date("Y-m-d");?>' /></td>
          </tr>
          <tr valign="baseline">
...
function Numberofdays(){
           ...
           t1=document.getElementById("SpecialLeave_Taken_Date").value;
...
document.getElementById("SpecialLeaveDays").value=_Diff;
...
-->
  ...
  <div id="sidecontent">
    ...
    <br /> <br />
    <br />
    <a href="../Proclamation/Ethiopian Labour Law Pro.377(English).htm#a81" target="_blank">Special Leave Labour Law</a>
