...
<table width="305" height="55" border="0" align="center">
  <tr>
    <td><?php if ($pageNum_RSMaternityLeaveDisplay > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RSMaternityLeaveDisplay=%d%s", $currentPage, 0, $queryString_RSMaternityLeaveDisplay); ?>"><img src="First.gif" /></a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_RSMaternityLeaveDisplay > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RSMaternityLeaveDisplay=%d%s", $currentPage, max(0, $pageNum_RSMaternityLeaveDisplay - 1), $queryString_RSMaternityLeaveDisplay); ?>"><img src="Previous.gif" /></a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_RSMaternityLeaveDisplay < $totalPages_RSMaternityLeaveDisplay) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_RSMaternityLeaveDisplay=%d%s", $currentPage, min($totalPages_RSMaternityLeaveDisplay, $pageNum_RSMaternityLeaveDisplay + 1), $queryString_RSMaternityLeaveDisplay); ?>"><img src="Next.gif" /></a>
        <?php } // Show if not last page ?></td>
    <td><?php if ($pageNum_RSMaternityLeaveDisplay < $totalPages_RSMaternityLeaveDisplay) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_RSMaternityLeaveDisplay=%d%s", $currentPage, $totalPages_RSMaternityLeaveDisplay, $queryString_RSMaternityLeaveDisplay); ?>"><img src="Last.gif" /></a>
    <?php } // Show if not last page ?></td>
  </tr>
</table>
</body>
