...
<table width="282" height="53" border="0" align="center">
  <tr>
    <td><?php if ($pageNum_RSSpecialLeaveDisplay > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RSSpecialLeaveDisplay=%d%s", $currentPage, 0, $queryString_RSSpecialLeaveDisplay); ?>"><img src="First.gif" /></a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_RSSpecialLeaveDisplay > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RSSpecialLeaveDisplay=%d%s", $currentPage, max(0, $pageNum_RSSpecialLeaveDisplay - 1), $queryString_RSSpecialLeaveDisplay); ?>"><img src="Previous.gif" /></a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_RSSpecialLeaveDisplay < $totalPages_RSSpecialLeaveDisplay) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_RSSpecialLeaveDisplay=%d%s", $currentPage, min($totalPages_RSSpecialLeaveDisplay, $pageNum_RSSpecialLeaveDisplay + 1), $queryString_RSSpecialLeaveDisplay); ?>"><img src="Next.gif" /></a>
        <?php } // Show if not last page ?></td>
    <td><?php if ($pageNum_RSSpecialLeaveDisplay < $totalPages_RSSpecialLeaveDisplay) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_RSSpecialLeaveDisplay=%d%s", $currentPage, $totalPages_RSSpecialLeaveDisplay, $queryString_RSSpecialLeaveDisplay); ?>"><img src="Last.gif" /></a>
        <?php } // Show if not last page ?></td>
  </tr>
</table>
</body>
