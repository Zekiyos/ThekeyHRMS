...
$query_RSAnnualLeaveDisplay = "SELECT * FROM annual_leave ORDER BY ID";
...
<table width="340" border="0" align="center">
  <tr>
    <td><?php if ($pageNum_RSAnnualLeaveDisplay > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RSAnnualLeaveDisplay=%d%s", $currentPage, 0, $queryString_RSAnnualLeaveDisplay); ?>"><img src="First.gif" /></a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_RSAnnualLeaveDisplay > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RSAnnualLeaveDisplay=%d%s", $currentPage, max(0, $pageNum_RSAnnualLeaveDisplay - 1), $queryString_RSAnnualLeaveDisplay); ?>"><img src="Previous.gif" /></a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_RSAnnualLeaveDisplay < $totalPages_RSAnnualLeaveDisplay) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_RSAnnualLeaveDisplay=%d%s", $currentPage, min($totalPages_RSAnnualLeaveDisplay, $pageNum_RSAnnualLeaveDisplay + 1), $queryString_RSAnnualLeaveDisplay); ?>"><img src="Next.gif" /></a>
        <?php } // Show if not last page ?></td>
    <td><?php if ($pageNum_RSAnnualLeaveDisplay < $totalPages_RSAnnualLeaveDisplay) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_RSAnnualLeaveDisplay=%d%s", $currentPage, $totalPages_RSAnnualLeaveDisplay, $queryString_RSAnnualLeaveDisplay); ?>"><img src="Last.gif" /></a>
        <?php } // Show if not last page ?></td>
  </tr>
</table>
</body>
