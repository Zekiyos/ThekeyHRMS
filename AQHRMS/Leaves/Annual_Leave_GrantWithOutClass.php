	...
	{				
				...
				while($row = mysql_fetch_array($resultWM, MYSQL_ASSOC))
				{
							...
							///	$TotalAL=round(($initialALdays)*($row['Workingmonths'])/12);
								...
								$TotalAL=round(($initialALdays)*($row['Workingmonths'])/12); //calculating annual leave  for lessthan a year
								...
								for($LastYearLeaveday;$LastYearLeaveday>=14;$LastYearLeaveday--)
								{
								...
								else  //if it not float mean actula year
								{
								$TotalAL=0;
