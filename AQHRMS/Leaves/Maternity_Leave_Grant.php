...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	...
	if ($_POST['MaternityLeaveDays']<0)
	{
	echo"<script type=\"text/javascript\"> alert('The Report Back date is less than the Leave granted date'); </script>";
	}
	else
	if ($obj_leave->CHK_Leave_Existance($_GET['ID'],$_POST['MaternityLeave_Taken_Date'])!=false)
	{   $Leave_Taken_Date=$_POST['MaternityLeave_Taken_Date'];
		$LeaveType=$obj_leave->CHK_Leave_Existance($_GET['ID'],$_POST['MaternityLeave_Taken_Date']);
		echo"<script type=\"text/javascript\"> alert('This Employee is already taken $LeaveType On Date $Leave_Taken_Date.'); </script>";
	}
	else
	{
  $insertSQL = sprintf("INSERT INTO maternity_leave (ID, FirstName, MiddelName, LastName,Department, MaternityLeaveDays, MaternityLeave_Taken_Date, ReportOn) VALUES (%s, %s,%s, %s, %s, %s, %s, %s)",
  ...
  <div id="mainContent"><!-- InstanceBeginEditable name="MainContent" -->
        ...
        <blockquote>
          <blockquote>
            <blockquote>
              <blockquote>
<font color="#FF6600" size="+1"><?php  echo $obj_lang->get('Maternity Leave Grant Form', $lang); ?></font>

			   <?php //include("Select_ID4MaternityLeaveGrant.php");?>
               <?php
$_GET['TableName']="employee_personal_record";

$_GET['OpenPage']="maternity_Leave_Grant";
      ...
      <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1" onmouseover="Numberofdays()">
        <table align="center" bgcolor="#EBEBEB">
          <tr valign="baseline">
            ...
            <td><input type="text" name="ID" value="<?php if(isset($_GET['ID'] )) echo $_GET['ID']; ?>" size="10" readonly="readonly" /></td>
          </tr>
          <tr valign="baseline">
          ...
          <tr valign="baseline">
            ...
            <td><input type="text" id="MaternityLeaveDays" name="MaternityLeaveDays" value="" size="4" readonly="readonly" /> 
          ...
          <tr valign="baseline">
            <td height="34" align="right" nowrap="nowrap"><?php  echo $obj_lang->get('Maternity Leave Taken Date', $lang); ?>:</td>
            <td><input type="text" id="MaternityLeave_Taken_Date" name="MaternityLeave_Taken_Date" value="<?php echo date("Y-m-d"); ?>" size="15" onclick='scwShow(this,event);' /></td>
          </tr>
          <tr valign="baseline">
		...
		<input type="text"  id="ReportOn" name="ReportOn" onfocus="Numberofdays()" onblur="Numberofdays()"  onclick='scwShow(this,event);' value='<?php echo date("Y-m-d");?>' /></td>
          </tr>
          <tr valign="baseline">
            <td nowrap="nowrap" align="right">&nbsp;</td>
            <td><input type="submit" onmousemove="Numberofdays()" value="<?php  echo $obj_lang->get('Grant Maternity leave', $lang); ?>" onclick="return confirm('Are you sure you want to Grant Matternity Leave for this Employee?')"  /></td>
          </tr>
        </table>
...
//
           ...
           t1=document.getElementById("MaternityLeave_Taken_Date").value;
...
document.getElementById("MaternityLeaveDays").value=_Diff;
...
-->
  ...
  <div id="sidecontent">
    ...
    <br /> <br />
    <br />
    <a href="../Proclamation/Ethiopian Labour Law Pro.377(English).htm#a88" target="_blank"><?php  echo $obj_lang->get('Maternity Leave Labour Law', $lang); ?></a>
