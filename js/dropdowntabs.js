...
function cleardefaultdata(){
    document.getElementById('zoom_query').onfocus=function(){
        if (this.value=='Search JavaScript Kit')
            this.value=''
    }
}

function disabledefaultsearch(){
    document.getElementById('jksitesearch').onsubmit=function(){
        var searchvalue=document.getElementById('zoom_query').value
        if (searchvalue=='Search JavaScript Kit' || searchvalue==''){
            alert('You haven\'t entered a search phrase yet!')
            return false
        }
    }
}
...
function setdefaultcategory(){
    var catsection=document.getElementById('jksitesearch_cat')
    var categories=catsection.getElementsByTagName('input')
    var docpath=location.pathname
    for (var i=0; i<categories.length; i++)
        categories[i].checked=false
    if (/(script\/)|(cutpastejava.shtml)/i.test(docpath))
        categories[1].checked=true
    else if (/(jsref\/)|(domref\/)|(filters\/)/i.test(docpath))
        categories[3].checked=true
    else if (/(javatutors\/)|(dhtmltutors\/)|(howto\/)/i.test(docpath))
